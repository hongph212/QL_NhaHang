...
namespace QuanLyNhaHang
{
    public partial class UserControlNhanVien : UserControl
    {
        ...
        private void btn_LuuThemNV_Click(object sender, EventArgs e)
        {
            ...
            if (txt_MaNV.Text != string.Empty && txt_HoTen.Text != string.Empty && txt_SDT.Text != string.Empty && txt_DiaChi.Text != string.Empty && !String.IsNullOrEmpty(txt_NgaySinh.Text) && !String.IsNullOrEmpty(txt_NgayVL.Text))
            {
        ...
        private void cbo_SearchChucVu_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string chucvu = cbo_SearchChucVu.SelectedIndex.ToString();
                ...
                {
                    ((DataTable)grid_DSNV.DataSource).Rows.Clear();
                    grid_DSNV.DataSource = dt.LayDS_TheoChucVu(chucvu);
                }
            }
            catch
            {
        ...
        {
            if (grid_DSNV.CurrentRow != null)
            {
                txt_MaNV.Text = grid_DSNV.CurrentRow.Cells[0].Value.ToString();
                txt_HoTen.Text = grid_DSNV.CurrentRow.Cells[1].Value.ToString();
                txt_User.Text = grid_DSNV.CurrentRow.Cells[2].Value.ToString();
                txt_SDT.Text = grid_DSNV.CurrentRow.Cells[3].Value.ToString();
                txt_DiaChi.Text = grid_DSNV.CurrentRow.Cells[4].Value.ToString();
                ...
                if (grid_DSNV.CurrentRow.Cells[6].Value.ToString() == "Nam")
                    rdo_Nam.Checked = true;
                else if (grid_DSNV.CurrentRow.Cells[6].Value.ToString() == "Nữ")
                    rdo_Nu.Checked = true;

                manv = grid_DSNV.CurrentRow.Cells[0].Value.ToString();
            }
        }
        private void grid_DSNV_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            e.Cancel = true;
        }

        private void grid_DSNV_MouseLeave(object sender, EventArgs e)
        {
            if (grid_DSNV.Rows.Count > 0)
            {
                string hoten = grid_DSNV.CurrentRow.Cells[1].Value.ToString();
                string user = grid_DSNV.CurrentRow.Cells[2].Value.ToString();
                string sdt = grid_DSNV.CurrentRow.Cells[3].Value.ToString();
                string diachi = grid_DSNV.CurrentRow.Cells[4].Value.ToString();
                string chucvu = grid_DSNV.CurrentRow.Cells[7].Value.ToString();
                string ngaysinh = grid_DSNV.CurrentRow.Cells[5].Value.ToString();
                string ngayvl = grid_DSNV.CurrentRow.Cells[8].Value.ToString();

                string gioitinh = grid_DSNV.CurrentRow.Cells[6].Value.ToString();


                dt.SuaNV(manv, hoten, sdt, diachi, ngaysinh, gioitinh, chucvu, ngayvl);
            }
        }
