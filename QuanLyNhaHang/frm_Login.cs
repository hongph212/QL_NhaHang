...
namespace QuanLyNhaHang
{
    public partial class frm_Login : Form
    {
        ...
        {
            string user = txt_LoginUser.Text;
            string pass = txt_LoginPass.Text;

            if (txt_LoginUser.Text != "" && txt_LoginPass.Text != "")
            {
                if (dt.dangnhap(user, pass) != null)
                {
                    if (dt.layquyennhanvien(user) == 1)
                    {
                        Login.User = txt_LoginUser.Text;
                        manv = dt.dangnhap(user, pass);
